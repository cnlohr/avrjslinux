...
Term.prototype.refresh = function (ka, la) {
    ...
    for (y = ka; y <= la; y++) {
        ta = y + this.y_disp;
        if (ta >= this.cur_h) ta -= this.cur_h;
        ia = this.lines[ta];
        na = "";
        w = this.w;
...
Term.prototype.write = function (ua) {
    ...
    for (i = 0; i < ua.length; i++) {
        ...
        case Ba:
            if (c >= 48 && c <= 57) {
                ...
                case 72:
                    {
                        ...
                        if (this.esc_params.length >= 2) Ca = this.esc_params[1] - 1;
                        else Ca = 0;
                        if (ta < 0) ta = 0;
                        else if (ta >= this.h) ta = this.h - 1;
                        if (Ca < 0) Ca = 0;
                        else if (Ca >= this.w) Ca = this.w - 1;
                        this.x = Ca;
                        this.y = ta;
                    }
                    break;
                case 74:
                    ...
                    for (j = this.y + 1; j < this.h; j++) wa(this, 0, j);
                    break;
                case 75:
                    wa(this, this.x, this.y);
                    break;
		...
		case 63:  //?
		    break;
		case 115: //s (Save cursor position)
		    this.savedx = this.x;
		    this.savedy = this.y;
		    break;
		case 117: //u (Restore cursor position)
		    this.x = this.savedx;
		    this.y = this.savedy;
		    break;
                default:
